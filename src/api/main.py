...
def get_cache_key(substructure: str) -> str:
    """ключ для кеша на основе субструктуры"""
...
async def get_cached_search(substructure: str):
    ...
    try:
        ...
        if cached:
            logger.info(f"кеш для {substructure}")
...
async def save_to_cache(substructure: str, result: dict, ttl: int = 3600):
    ...
    try:
        ...
        logger.info(f"сохранили кеш для {substructure}")
    except Exception as e:
...
async def read_molecule(molecule_id: int, db: Session = Depends(get_db)):
    ...
    if molecule is None:
        raise HTTPException(status_code=404, detail=f"Молекула с id={molecule_id} не нашлась")
...
async def get_task_status(task_id: str):
    ...
    try:
        ...
        # задача в процессе - добавляем информацию о прогрессе
